<template>
    <div class="col-sm-6">
        <div class="panel panel-primary">
            ...
            <div class="panel-footer text-right" v-if="this.$parent.status == 'ready'">
...
<script>
    export default {
        ...
        computed: {
            enableOpen: function () {
                return this.$parent.status == 'open' && !this.item.event_open;
            },
        ...
        methods: {
            edit: function(event) {
                this.$http.patch('admin/items/'+this.item.id, this.item)
                    .then((response) => {
                        alert('수정 되었습니다.');
                    });
            },
            remove: function(event) {
                if (window.confirm('['+this.item.title+'] 삭제 하시겠습니까?')) {
                    this.$http.delete('admin/items/'+this.item.id)
                        .then((response) => {
                            this.$emit('remove');
                        });
                }
            ...
            eventOpen: function(event) {
                if (window.confirm('출석 이벤트를 시작 하시겠습니까?')) {
                    this.$http.patch('admin/items/'+this.item.id+'/event', {"open": true, "rank":this.item.event_rank})
                        .then((response) => {
            ...
            eventResult: function(event) {
                this.$http.patch('admin/items/'+this.item.id+'/event', {"open": false})
                    .then((response) => {
